        ...
        static void findPath()
        {
            ...
            while (openSet.Count > 0 && !foundSolution)
            {
                ...
                if (selectedNode.Turn >= board.LastTurnGenerated)
                {
                    ...
                    var lastTurn = openSet.UnorderedItems.MinBy(x => x.Element.Turn).Element.Turn;
                    board.RemoveUntilTurn(lastTurn - 1);
            ...
            if (!foundSolution)
            {
                //TODO if no solution found, get the furthest path
            }
        }
